...
base = "/content/drive/MyDrive/BTL_DeTai4/dataset"
...
data_yaml = "/content/drive/MyDrive/BTL_DeTai4/dataset/data.yaml"
```
→ Ngay sau cell này, dán cell định nghĩa `log_experiment` (xem mục "Ghi log thí nghiệm tự động" ở trên) — chạy 1 lần.

Kiểm tra `data.yaml` — nếu đường dẫn `train:`/`val:` là đường dẫn tuyệt đối cũ (do Roboflow ghi sẵn đường dẫn lúc tải ở máy Track A), sửa lại thành tương đối `train/images`, `valid/images`, `test/images` cho khớp cấu trúc thư mục trên Drive.
